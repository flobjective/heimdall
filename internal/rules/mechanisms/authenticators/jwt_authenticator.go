...
func (a *jwtAuthenticator) renderFilePath(filePath string, tokenClaims map[string]any) (string, error) {
	if len(tokenClaims) == 0 || !strings.Contains(filePath, "{{") {
		return filePath, nil
	}
	...
	if !ok {
		return "", errorchain.NewWithMessage(heimdall.ErrInternal,
			"invalid or missing 'iss' claim for template rendering")
	}

	return tpl.Render(map[string]any{"TokenIssuer": issuer})
}

func (a *jwtAuthenticator) calculateCacheKeyForMetadata(
	metadata *oauth2.ServerMetadata, keyID string, tokenClaims map[string]any,
) string {
	digest := sha256.New()
